# Hi there! I'm Orlando Flores (orfloresti) 👋

### 🚀 Front-end Developer | Angular Specialist | Tech Enthusiast

I'm a passionate software developer based in **Tlaxcala, Mexico**, currently working in the **IT industry** with a focus on **Front-end engineering** and **Production Support**. I specialize in building scalable web applications and optimizing development workflows.

---

### 🛠️ My Tech Stack

* **Languages:** TypeScript, JavaScript (ES6+), Python, Rust (Learning)
* **Frameworks:** Angular (Expertise), React (Familiar)
* **Tools & Devops:** Git, Scripting, Production Support, Web Performance
* **Current Goal:** Reaching **C1 English proficiency** to collaborate on global projects.

---

### 🌐 Beyond the Code

When I'm not writing scripts or debugging production issues, you can find me:
* 🌱 Caring for my **Alocasia** and **Monstera** collection.
* 📚 Reading about **Mindset** or classic **Jules Verne** novels.
* ♟️ ♟️ **Chess Enthusiast:** Catch me playing online at [Chess.com](https://www.chess.com/member/orfloresti) or [Lichess](https://lichess.org/@/orfloresti).

---

### 📫 Connect with me:

* **Personal Website:** [orfloresti.dev](https://orfloresti.dev/)
* **LinkedIn:** [Your LinkedIn Link Here]

---

*"Success is a mindset. Keep building."*